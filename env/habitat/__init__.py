...
from .exploration_env import Exploration_Env, DMExploration, DMExplorationExtras
...
def dm_make_env_fn(args, seed: int = None ):
    # TODO: add agent height and radius
    basic_config = cfg_env(config_paths=["env/habitat/habitat_api/configs/" + args.task_config])
    basic_config.defrost()
    basic_config.DATASET.SPLIT = args.split
    basic_config.freeze()
    scenes = PointNavDatasetV1.get_scenes_to_load(basic_config.DATASET)
    # env config
    config_env = cfg_env(config_paths=["env/habitat/habitat_api/configs/" + args.task_config])
    config_env.defrost()
    if len(scenes) > 0: config_env.DATASET.CONTENT_SCENES = scenes
    gpu_id = min(torch.cuda.device_count() - 1, 0)
    ...
    if not args.use_map or not args.nested_action:
        env = DMExploration(args=args, config_baseline=config_baseline, config_env=config_env, dataset=dataset, rank=seed)
    else:
        env = DMExplorationExtras(args=args, config_baseline=config_baseline, config_env=config_env, dataset=dataset, rank=seed)
    env.seed(seed)
    return env 
